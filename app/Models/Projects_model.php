...
class Projects_model extends Crud_model
{
    ...
    function count_project_statuses($options = array())
    {
        $projects_table = $this->db->prefixTable('projects');
        $project_members_table = $this->db->prefixTable('project_members');
        ...
        FROM $projects_table
              $extra_join    
        WHERE $projects_table.deleted=0 $extra_where
        GROUP BY $projects_table.status";
    ...
    function chart_project_open($options = array())
    {
        $projects_table = $this->db->prefixTable('projects');
        $project_members_table = $this->db->prefixTable('project_members');
        $year = get_my_local_time("Y");
        $beforeyear =  $beforeyear = $year - 1;
        $extra_join = "";
        $extra_where = "";
        $user_id = get_array_value($options, "user_id");
        if ($user_id) {
        ...
        // $openbeforeyear = "SELECT $projects_table.status, count($projects_table.id) as total
        // FROM $projects_table
        //       $extra_join    
        // WHERE $projects_table.deleted=0 AND $projects_table.status='open' AND YEAR($projects_table.start_date)=$beforeyear $extra_where
        // GROUP BY $projects_table.status";

        $opens = "SELECT $projects_table.status as statuses, count($projects_table.id) as total, MONTH($projects_table.start_date) AS month
        FROM $projects_table
              $extra_join    
        WHERE $projects_table.deleted=0 AND $projects_table.status='open' AND YEAR($projects_table.start_date)=$year $extra_where
        GROUP BY $projects_table.status, MONTH($projects_table.start_date)";

        $info = new \stdClass();
        $info->opens = $this->db->query($opens)->getResult();

        return $info;
    }


    function chart_project_openbeforeyear($options = array())
    {
        $projects_table = $this->db->prefixTable('projects');
        $project_members_table = $this->db->prefixTable('project_members');
        $year = get_my_local_time("Y");
        ...
        FROM $projects_table
              $extra_join    
              WHERE $projects_table.deleted=0 AND ($projects_table.status='open' OR  $projects_table.status='completed') AND YEAR($projects_table.start_date)=$beforeyear $extra_where
        GROUP BY $projects_table.status";

        $result = $this->db->query($openbeforeyear)->getResult();
    ...
    function count_client_projects()
    {
        $projects_table = $this->db->prefixTable('projects');

        $sql = "SELECT $projects_table.status, COUNT($projects_table.id) as total
        FROM $projects_table
        WHERE $projects_table.deleted=0 AND ($projects_table.status='open' OR  $projects_table.status='completed' OR  $projects_table.status='hold' OR  $projects_table.status='canceled')
        GROUP BY $projects_table.status";
        $result = $this->db->query($sql)->getResult();
