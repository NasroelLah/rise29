...
if (!function_exists('project_stataus_widget')) {

    function project_stataus_widget($user_id = 0)
    {
        $ci = new Security_Controller(false);
        ...
        $view_data["project_statuses"] = $ci->Projects_model->count_project_statuses($options);

        $template = new Template();
        return $template->view("projects/chart/project_status_widget", $view_data);
    }
}
...
if (!function_exists('chart_project_open_widget')) {

    function chart_project_open_widget($user_id = 0)
    {
        $ci = new Security_Controller(false);
        ...
        $info = $ci->Projects_model->chart_project_open($options);
        $opens = array();
        $opens_array = array();
        ...
        foreach ($info->opens as $open) {
            $opens[$open->month] = $open->total;
        }
        $infos = $ci->Projects_model->chart_project_openbeforeyear($options);
        $view_datas =  $infos->open;

        $runningSum = $view_datas;
        foreach ($opens as $number) {
            $runningSum += $number;
            $tot[] = $runningSum;
        }
        foreach ($tot as $key => $open) {
            $opens_array[] = $open;
        }


        $view_data["project_open"] = json_encode($opens_array);

        $template = new Template();
        return $template->view("projects/chart/open_projects_chart", $view_data);
    }
}
...
// if (!function_exists('open_projects_chart')) {

//     function open_projects_chart($user_id = 0) {
//         $ci = new Security_Controller(false);
